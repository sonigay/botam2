...
async def on_ready():
	...
	await client.change_presence(status=discord.Status.dnd, activity=discord.Game(name="군터6섭 보탐안내", type=1), afk=False)
				...
				if msg.channel.id == int(basicSetting[11]) : #### 정산채널 채널ID 값넣으면 됨
					...
					if message.content.startswith(command[12]):
						if basicSetting[10] !="" and basicSetting[12] !="" and basicSetting[14] !="" and basicSetting[15] !="" and basicSetting[16] !=""  :
							...
							embed = discord.Embed(
									description= '```' + SearchID + ' 님이 받을 다이야는 총 ' + result + ' 입니다!!```',
									color=0xff00ff
									)
